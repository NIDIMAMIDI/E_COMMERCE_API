...
export const authValidator = (req, res, next)=>{
    ...
    password: Joi.string().regex(strongPasswordRegex).required().messages({
        ...
        "string.pattern.base": "Password must be strong. At least one upper case alphabet, one lower case alphabet, one digit, one special character, and between 8 and 15 characters long",
...
validateSchema(schema, req.body, res);
next();
